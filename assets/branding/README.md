...
- **Boot splash + LUKS unlock come from the initramfs, not from `/usr`.** The
  ...
  - **Fresh install** (Anaconda / bootc-installer from the image): the
    initramfs is built from the bluecat `/usr` -> boot splash shows bluecat.
  ...
  - We intentionally do **not** regenerate the initramfs inside the image build
    (baking an initramfs into a bootc image is a known source of boot issues and
    bloats the image). The boot-splash branding is cosmetic and initramfs-bound.
