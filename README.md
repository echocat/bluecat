...
> are rendered from the *initramfs*, not from `/usr`. A **fresh install** shows
> the bluecat splash. When **rebasing onto an existing Fedora system**, the old
> initramfs (with Fedora branding) is kept until it is regenerated; run
